    ...
    <div className="bg-white">
      ...
      <section className="relative overflow-hidden bg-[var(--color-navy-950)]">
        ...
        <div
          ...
          style={{
            backgroundImage: 'linear-gradient(var(--color-navy-700) 1px, transparent 1px), linear-gradient(90deg, var(--color-navy-700) 1px, transparent 1px)',
            backgroundSize: '48px 48px',
          }}
        />
        {/* Gold accent line */}
        <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-[var(--color-gold-500)] to-transparent" />

        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28">
          <div className="max-w-3xl">
            {/* Badge */}
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[var(--color-gold-500)]/30 bg-[var(--color-gold-500)]/10 mb-8">
              <span className="w-1.5 h-1.5 rounded-full bg-[var(--color-gold-400)]" />
              <span className="text-[var(--color-gold-300)] text-xs font-medium tracking-wide">
                INDIA'S FIRST COURT-PERFORMANCE LAWYER DATABASE
              </span>
            </div>
            ...
            >
              Find Lawyers by{' '}
              <span className="text-[var(--color-gold-400)]">Proven</span>{' '}
              Court Performance
            </h1>
            <p className="text-[var(--color-navy-200)] text-lg leading-relaxed mb-8 max-w-2xl">
              CourtCounsel analyses publicly available court records from Odisha High Court and District Courts to give you data-backed lawyer rankings — not just self-reported credentials.
            </p>

            {/* Search bar */}
            <div className="bg-white rounded-xl p-2 flex gap-2 shadow-2xl shadow-black/30 max-w-2xl">
              <div className="flex-1 flex items-center gap-3 px-3">
                <Search size={18} className="text-[var(--color-muted-foreground)] flex-shrink-0" />
                <input
                  type="text"
                  placeholder="Search by lawyer name, practice area, court, or city..."
                  className="flex-1 text-sm text-[var(--color-navy-900)] placeholder:text-[var(--color-muted-foreground)] outline-none bg-transparent"
                  value={query}
              ...
              <button
                ...
                className="bg-[var(--color-navy-900)] text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-[var(--color-navy-700)] transition-colors"
              >
                Search
              </button>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {['Criminal Law', 'Civil Rights', 'Corporate Law', 'Family Law', 'Land Acquisition'].map(area => (
                <button
                  key={area}
                  onClick={() => navigate('search', area)}
                  className="text-xs px-3 py-1 rounded-full border border-[var(--color-navy-700)] text-[var(--color-navy-300)] hover:border-[var(--color-gold-500)] hover:text-[var(--color-gold-400)] transition-colors"
                >
                  {area}
                </button>
              ))}
        ...
        {/* Stats bar */}
        <div className="relative border-t border-[var(--color-navy-800)]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                { value: '2,400+', label: 'Lawyers Indexed' },
                { value: '48,000+', label: 'Cases Analysed' },
                { value: '12', label: 'Courts Covered' },
                ...
                <div key={s.label} className="text-center">
                  ...
                  <div className="text-xs text-[var(--color-navy-400)] mt-1 uppercase tracking-wide">{s.label}</div>
                </div>
              ))}
            </div>
          </div>
